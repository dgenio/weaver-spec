...
- This repo is **docs + contracts only**. Never add runtime logic, adopter-facing CLI tools, or adopter-facing helper utilities. Build-time spec-maintenance tooling is permitted under `scripts/` (stdlib-only, never imported by `weaver_contracts`, never published). Two further CI-checked exceptions may use the schema-validation/signing deps: the conformance suite (`conformance/`, including `scoreboard.py`) and the single runnable example (`examples/reference_impl/`) — both never imported by the package and never published. Treat these as in-scope; see `AGENTS.md` for the full scope rule.
