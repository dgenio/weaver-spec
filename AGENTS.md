...
| `scripts/check_docs_counts.py` | Build-time tooling (stdlib only) that fails when a doc states a Core/Extended type count that no longer matches the schema filesystem (#118); contract-version strings are enforced separately by `check_version_consistency.py`; run by the `validate-docs-counts` CI job and the `docs-counts` pre-commit hook | When adding/removing a contract type, or when a doc states a derived count |
