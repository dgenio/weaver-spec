...
- **The conformance suite under `conformance/` is build-time / CI tooling**, not runtime code. Unlike `scripts/` it may use the schema-validation/signing dependencies (`jsonschema`, `referencing`, `PyYAML`, `jcs`, `cryptography`), because it defines what "spec-compliant" means as a runnable check (positive/negative corpus + executable invariant assertions + TraceBundle signature verification). The same constraints apply: it is never imported by `weaver_contracts` and never published. See [docs/CONFORMANCE.md](docs/CONFORMANCE.md). This also covers the scoreboard builder (`conformance/scoreboard.py`, #51) and its registry (`conformance/siblings.yaml`).
- **A single runnable example is permitted under `examples/reference_impl/`** (#76) as an explicit "example tier" exception to the docs-and-contracts rule. It is a CI-checked demonstration that constructs every Core artifact end-to-end; like `conformance/` it may use the schema-validation/signing dependencies, but it is **not** adopter-facing tooling, is never imported by `weaver_contracts`, and is never published. It exists so a newcomer can read one file — not as a utility shipped for adopters to call. Treat it as in-scope (do not "clean it up" as stray runtime code); any new runnable example must be added to the repo map below with that scope stated.
- **A single umbrella meta-package is permitted under `packaging/weaver-stack/`** (#80) as an explicit "packaging tier" exception. It ships **no code** — it only pins a known-compatible set of the stack so adopters can `pip install weaver-stack`. It is published to PyPI as `weaver-stack` (a second published artifact alongside `weaver_contracts`); its pinned set is the executable form of `compatibility.yaml` and is kept in sync by `scripts/validate_meta_package.py` (the `validate-meta-package` CI job). It is never imported by `weaver_contracts`. This is the **only** packaging artifact permitted beyond `weaver_contracts`; do not add further published packages without amending this rule.
...
| `docs/SCOREBOARD.md` / `docs/scoreboard.md` / `docs/badges/` | Public conformance scoreboard (#51): participation guide, the generated scoreboard page, and per-repo shields.io badge endpoints | When changing the scoreboard, the registry, or the badge format |
...
| `scripts/generate_schema_bundle.py` / `contracts/bundles/weaver-contracts.bundle.json` | Stdlib-only build-time generator and checked-in compound JSON Schema distribution artifact; the bundle is outside `contracts/json/` and is not a Core contract | When schemas or the content-addressed index change, or when validating offline distribution |
...
| `conformance/` | Build-time / CI conformance suite: `run.py` runner (corpus mode + `--bundle` single-bundle mode + `--emit-result`/`--emit-badge`), `scoreboard.py` (#51 scoreboard builder), `siblings.yaml` (scoreboard registry), `corpus.yaml`, `invariants.yaml` (executable I-01/I-02/I-04/I-06 assertions), negative fixtures, and a signed TraceBundle fixture + test keyring. Consumes the schema-validation/signing deps; never imported by `weaver_contracts`, never published. | When changing what spec-compliance verifies, or adding fixtures/invariant checks/scoreboard entries |
